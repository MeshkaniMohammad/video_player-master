...
@property(nonatomic, readonly) bool isLockedDeviceOrActiveAirplay;
...
- (void) handleEnteredBackground {
    if ([[UIScreen mainScreen] brightness] == 0) {
        if (_isPlaying) {
            _isLockedDeviceOrActiveAirplay = true;
        }
    } else {
        if ([self isAirplayOn]) {
            _isLockedDeviceOrActiveAirplay = true;
...
- (void) handleEnteredForeground {
    _isLockedDeviceOrActiveAirplay = false;
}
...
- (void)itemDidPlayToEndTime:(NSNotification*)notification {
    if (_isLooping) {
        ...
        [self setupMPNowPlayingInfoCenter:kCMTimeZero];
    } else {
        if (_eventSink) {
...
- (instancetype)initWithURL:(NSURL*)url frameUpdater:(FLTFrameUpdater*)frameUpdater {
    ...
    _videoTitle = asset.URL.lastPathComponent;
...
- (instancetype)initWithPlayerItem:(AVPlayerItem*)item frameUpdater:(FLTFrameUpdater*)frameUpdater {
    ...
    MPSkipIntervalCommand *skipBackwardIntervalCommand = [_remoteCommandCenter skipBackwardCommand];
    [skipBackwardIntervalCommand setEnabled:YES];
    [skipBackwardIntervalCommand addTarget:self action:@selector(skipBackwardEvent:)];
    skipBackwardIntervalCommand.preferredIntervals = @[@(15)];  // Set your own interval

    MPSkipIntervalCommand *skipForwardIntervalCommand = [_remoteCommandCenter skipForwardCommand];
    skipForwardIntervalCommand.preferredIntervals = @[@(15)];  // Max 99
    [skipForwardIntervalCommand setEnabled:YES];
    [skipForwardIntervalCommand addTarget:self action:@selector(skipForwardEvent:)];

    MPRemoteCommand *pauseCommand = [_remoteCommandCenter pauseCommand];
    [pauseCommand setEnabled:YES];
    [pauseCommand addTarget:self action:@selector(playOrPauseEvent:)];

    MPRemoteCommand *playCommand = [_remoteCommandCenter playCommand];
    [playCommand setEnabled:YES];
    [playCommand addTarget:self action:@selector(playOrPauseEvent:)];

    MPRemoteCommand *togglePlayPauseCommand = [_remoteCommandCenter togglePlayPauseCommand];
...
-(MPRemoteCommandHandlerStatus) playOrPauseEvent: (MPSkipIntervalCommandEvent *) playOrPauseEvent {
    if (@available(iOS 10.0, *)) {
        if (_player.timeControlStatus == AVPlayerTimeControlStatusPlaying) {
            ...
            _isPlaying = true;
            [self updatePlayingState];
            [self setupMPNowPlayingInfoCenter: _player.currentTime];
        }
    }
...
-(MPRemoteCommandHandlerStatus) skipBackwardEvent: (MPSkipIntervalCommandEvent *) skipEvent {
    ...
    if (CMTimeGetSeconds(newTime) < 0) {
      [ self seekTo: CMTimeMakeWithSeconds(0,1)];
    } else {
        [ self seekTo:newTime];
    }
    return MPRemoteCommandHandlerStatusSuccess;
...
- (void)updatePlayingState {
    ...
    if (_isPlaying) {
       if (@available(iOS 10.0, *)) {
        ...
        if (_isLockedDeviceOrActiveAirplay) {
            if (@available(iOS 10.0, *)) {
                [_player playImmediatelyAtRate:1.0];
                _isLockedDeviceOrActiveAirplay = false;
            } else {
...
- (void) setupMPNowPlayingInfoCenter: (CMTime)location {
    NSMutableDictionary *playingInfo = [NSMutableDictionary dictionary];
    int duration = CMTimeGetSeconds( [[_player currentItem] duration]);
    playingInfo[MPMediaItemPropertyPlaybackDuration] = [NSNumber numberWithInt: duration];
    int position = CMTimeGetSeconds(location);
    playingInfo[MPNowPlayingInfoPropertyElapsedPlaybackTime] = [NSNumber numberWithInt: position];
    playingInfo[MPMediaItemPropertyAlbumTitle] = _videoTitle;
...
- (void)seekTo:(CMTime)location {
    [_player seekToTime:location
        toleranceBefore:kCMTimeZero
         toleranceAfter:kCMTimeZero];
    [self setupMPNowPlayingInfoCenter: location];
}
...
- (void)seekTo:(FLTPositionMessage*)input error:(FlutterError**)error {
    FLTVideoPlayer* player = _players[input.textureId];
    [player seekTo:CMTimeMake([input.position intValue], 1000)];
}
